...
data class Resource<ResultType>( var status : Status,var data: ResultType? = null,var error: VolleyError? = null, var isPaginatedLoading: Boolean = false) {
    ...
    companion object {
         ...
         */
        fun <ResultType> success(data: ResultType, isPaginatedLoading: Boolean = false): Resource<ResultType> = Resource(Status.SUCCESS,data)
        ...
        fun <ResultType> error(apiError: VolleyError?, isPaginatedLoading: Boolean = false): Resource<ResultType> = Resource(Status.ERROR, error = apiError)
    }
}
